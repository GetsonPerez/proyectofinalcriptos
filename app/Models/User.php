 ...
 * @property-read \Illuminate\Database\Eloquent\Collection<int, \App\Models\UserBilletera> $billeteras
 * @property-read int|null $billeteras_count
...
//        dd($this->id,$opcionesDirectas->toArray(),$opcionesPorRol->toArray(),$allOptions->toArray());
    ...
    {
        return $this->hasMany(UserBilletera::class);
     ...
     * @throws Exception
     */
    public function transaccionBilletera(CriptomonedaTransaccion $transaccion): UserBilletera
    {

        $criptoMoneda = $transaccion->criptomoneda;
        $tipo = $transaccion->tipo;
        $cantidad = $tipo == CriptomonedaTransaccion::TIPO_COMPRA ? $transaccion->cantidad : -$transaccion->cantidad;



        $billetera = $this->billeteras()->where('criptomoneda_id',$criptoMoneda->id)->first();

        if ($tipo == CriptomonedaTransaccion::TIPO_VENTA)
        {
            if (!$billetera)
            {
                throw new Exception('No tienes ' . $criptoMoneda->nombre . ' en tu billetera.');
            }

            if (abs($cantidad) > $billetera->saldo)
            {
                throw new Exception('Saldo insuficientes: Solo tienes ' . $billetera->saldo . ' de ' . $criptoMoneda->nombre . ' en tu billetera.');
            }
        }
        ...
        if(!$billetera)
        {
            $billetera = new UserBilletera();
            $billetera->user_id = $this->id;
            $billetera->criptomoneda_id = $criptoMoneda->id;
            $billetera->saldo = 0;
            $billetera->save();
        }
